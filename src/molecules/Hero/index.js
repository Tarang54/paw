...
function Hero({index}) {
  ...
  return (
    <div className='flex flex-row justify-between items-center py-[40px] px-12 max-w-[1100px] m-auto' style={{marginBottom: HeroData[index].bottomMargin}}>
      <div className="flex flex-col gap-6 justify-around" style={{width: HeroData[index].leftWidth}}>
        ...
        <div className="flex flex-row flex-wrap items-center gap-2 justify-start">
            ...
            {HeroData[index].btn2 && <Button onClick={() => console.log('clicked')} customText={HeroData[index].btn2} customStyles={{backgroundColor: "#1F72EA", padding:"0 12px", minWidth: "130px", color: "white", borderRadius: "8px", width: "40%", height: "40px"}}/>}
        </div>
      </div>
